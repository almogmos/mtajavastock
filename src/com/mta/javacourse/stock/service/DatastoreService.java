...
//import il.ac.mta.model.Account;
...
*/
	...
	private StockStatus fromStockEntry(Entity stockEntity) {
		StockStatus ret = new StockStatus(null, 0, 0, null, null, 0);
		ret.setSymbolName((String) stockEntity.getProperty(SYMBOL));
	...
	private Entity stockToEntity(StockStatus stock) {
		...
		Key key = KeyFactory.createKey(parent, NAMESPACE_STOCK, stock.getSymbolName());

		Entity entity = new Entity(key);
		entity.setProperty(SYMBOL, stock.getSymbolName());
	...
	private Entity portfolioToEntity(Portfolio portfolio) {
		...
		for (int i = 0; i < stocks.length; i++) {
			...
			if(stock != null)
				symbols.add(stock.getSymbolName());
		}
	...
	private void updateStock(StockStatus stockStatus) {
		...
		Filter fSymbol = new FilterPredicate(SYMBOL, FilterOperator.EQUAL, stockStatus.getSymbolName());
		final long oneDay = TimeUnit.DAYS.toMillis(1);
